...
NOOP is a local-first iPhone app for a WHO0P 4.0 strap. It connects directly to
the strap over Bluetooth, stores the data on the phone, and builds a WHO0P-style
...
on the App Store and does not require a WHO0P cloud account.

> NOOP is an independent project. It is not affiliated with, endorsed by, or
> connected to WHO0P, Inc. It is also not a medical device. Recovery, strain,
...
- Connects to a WHO0P 4.0 over Bluetooth.
- Streams live heart rate and R-R interval data.
- Offloads historical WHO0P sensor data into a local SQLite store.
- Displays a full-screen iPhone dashboard with Today, Workouts, Status, and More.
- Calculates recovery locally from WHO0P sleep, HRV, resting HR, respiratory
...
- Detects and plots sleep from WHO0P HR, R-R, respiration, and motion data.
...
- Shows steps from WHO0P-derived step data when available, otherwise estimated
  from WHO0P motion.
...
Sleep is derived locally from WHO0P data. NOOP detects the main overnight sleep
...
Strain is a 0-21 cardiovascular load estimate from WHO0P heart-rate data.
...
Sleep detection uses WHO0P heart rate, R-R intervals, respiration, and motion.
...
- WHO0P data is stored on the phone in SQLite.
- The app does not require a WHO0P account.
...
7. Keep the official WHO0P app closed while pairing if the strap is not showing
   up in NOOP.
...
StrandiOS/
  ...
  BLE/                 WHO0P scan/connect/live BLE handling
...
Packages/
  WHO0PProtocol/       WHO0P frame parsing, commands, CRC, protocol types
  WHO0PStore/          Local SQLite persistence via GRDB
...
- WHO0P 4.0 is the active target.
...
be wrong, incomplete, or inconsistent with the official WHO0P app. Do not use it
