    ...
    WarbFitCard {
        Column(verticalArrangement = Arrangement.spacedBy(Metrics.gap)) {
            Row(verticalAlignment = Alignment.Top) {
                ...
                Text(trailing, style = WarbFitType.number(18f), color = if (n > 0) accent else Palette.textTertiary)
            }
...
private fun SinglePoint(value: Double, accent: Color, fmt: (Double) -> String) {
    Box(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Text(fmt(value), style = WarbFitType.number(34f), color = accent)
        }
    }
}
...
private fun EmptyChart() {
    Box(
        ...
        Text("No readings recorded.", style = WarbFitType.subhead, color = Palette.textTertiary)
    }
}
