    ...
    ScreenScaffold(
        ...
        SettingsSection(
            ...
            Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                Text("When I double-tap", style = WarbFitType.body, color = Palette.textPrimary)
        ...
        SettingsSection(
            ...
            title = "Smart alarm",
            blurb = "Wake to a wrist buzz. This arms the strap's own firmware alarm, so it still fires if the phone is asleep or WarbFit is closed.",
        ) {
            ToggleRow(
            ...
            if (smartAlarm) {
                RowDivider()
                Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                    Text("Wake at", style = WarbFitType.body, color = Palette.textPrimary)
                    Spacer(Modifier.weight(1f))
                    Text("07:00", style = WarbFitType.number(15f), color = Palette.accent)
                }
                RowDivider()
                Text(
