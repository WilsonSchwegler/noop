...
/**
 * Imports a TRACKER CSV export (the four-CSV bundle, zipped or loose) into the local Room store.
 *
 * The CSV column mapping is reproduced faithfully: same columns -> same fields, same
 * header normalization + aliases, same
 * timezone-aware date parsing, same "(cal)" == kcal unit handling, same row-skip rules.
 *
 * Rows are mapped onto the verified Room entities (net.wilsonschwegler.warbfit.data) and upsert
 * through [TrackerRepository]. All TRACKER rows are written under deviceId "my-tracker".
 *
 ...
 *   workouts.csv              -> WorkoutRow(source = "my-tracker")
 *   journal_entries.csv       -> JournalEntry
 *
 * The whole pipeline is tolerant: missing files / columns / blank cells degrade gracefully.
 */
object TrackerCsvImporter {

    private const val TRACKER_DEVICE = "my-tracker"
    private const val SOURCE_LABEL = "TRACKER"
     ...
     *
     * Accepts either a `.zip` containing the TRACKER CSVs, or a single `.csv` (routed by
     ...
     * [deviceId] (defaults to "my-tracker"), then returns an [ImportSummary] keyed by table.
     */
    suspend fun importZip(
        context: Context,
        uri: Uri,
        repo: TrackerRepository,
        deviceId: String = TRACKER_DEVICE,
    ): ImportSummary {
        ...
        if (csvData.isEmpty()) {
            return ImportSummary.failure(
                SOURCE_LABEL,
                "No TRACKER CSVs found (expected physiological_cycles.csv, sleeps.csv, workouts.csv or journal_entries.csv)."
            )
        }
        ...
        if (daily.isEmpty() && sleepSessions.isEmpty() && workouts.isEmpty() && journal.isEmpty()) {
            return ImportSummary.failure(SOURCE_LABEL, "Export contained no usable TRACKER rows.")
        }

        repo.upsertDevice(deviceId, name = "TRACKER")
    ...
    /**
     * Return `[lowercasedFilename -> rawBytes]` for every recognised TRACKER CSV in the input.
    ...
    private fun loadCsvData(context: Context, uri: Uri): Map<String, ByteArray> {
        ...
        // First attempt: treat as a zip. TRACKER exports are zips; this also covers a .zip Uri
                            ...
                            if (base.endsWith(".csv")) {
                                ...
                                if (declared <= MAX_ENTRY_BYTES) {
                                    ...
                                    if (bytes != null && bytes.isNotEmpty()) {
                                        ...
                                        // what lets non-English TRACKER exports import (issue #3).
                                        val canonical = when {
                                            base in wanted -> base
        ...
        // name is unknown, sniff the header row to identify which TRACKER CSV it is.
    ...
    /**
     * Map a known localized TRACKER export filename to its canonical English name. TRACKER localizes
    ...
    private fun localizedAlias(base: String): String? = when (base) {
        // German (app.tracker.com → Daten exportieren)
    ...
    private fun parseCycles(table: CsvTable, deviceId: String): List<DailyMetric> {
        ...
        for (row in table.rows) {
            val tz = TrackerTime.tzOffsetMinutes(row["cycle_timezone"])
            val cycleStart = TrackerTime.parseEpochSeconds(row.cell("cycle_start_time"), tz)
            val cycleEnd = TrackerTime.parseEpochSeconds(row.cell("cycle_end_time"), tz)
            ...
            out.add(
                DailyMetric(
                    ...
                    remMin = remMin,
                    lightMin = lightMin,
                    // "awake_duration_min" -> disturbances slot (Tracker's disturbance count is
    ...
    private fun parseSleeps(table: CsvTable, deviceId: String): SleepParse {
        ...
        for (row in table.rows) {
            val tz = TrackerTime.tzOffsetMinutes(row["cycle_timezone"])
            val cycleStart = TrackerTime.parseEpochSeconds(row.cell("cycle_start_time"), tz)
            val sleepOnset = TrackerTime.parseEpochSeconds(row.cell("sleep_onset"), tz)
            val wakeOnset = TrackerTime.parseEpochSeconds(row.cell("wake_onset"), tz)
    ...
    private fun parseWorkouts(table: CsvTable, deviceId: String): List<WorkoutRow> {
        ...
        for (row in table.rows) {
            val tz = TrackerTime.tzOffsetMinutes(row["cycle_timezone"])
            val cycleStart = TrackerTime.parseEpochSeconds(row.cell("cycle_start_time"), tz)
            val workoutStart = TrackerTime.parseEpochSeconds(row.cell("workout_start_time"), tz)
            val workoutEnd = TrackerTime.parseEpochSeconds(row.cell("workout_end_time"), tz)
            ...
            out.add(
                WorkoutRow(
                    ...
                    endTs = if (endTs >= startTs) endTs else startTs,
                    sport = sport,
                    source = TRACKER_DEVICE, // required by the task: source = "my-tracker"
                    durationS = durationS,
