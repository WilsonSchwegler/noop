 ...
 * struct (Packages/TrackerProtocol/Sources/TrackerProtocol/Streams.swift) carrying the rows
 ...
 * stored on the per-row sample models — it is supplied to [TrackerRepository.insert]).
 */
...
/** Count of rows ACTUALLY inserted per stream (mirrors TrackerStore.insert return tuple). */
...
/**
 * Repository over [TrackerDatabase] / [TrackerDao]. The single seam the rest of the app uses
 * to read/write the local store. Port of TrackerStore's public surface (StreamStore.swift,
...
class TrackerRepository(private val dao: TrackerDao) {
    ...
    constructor(db: TrackerDatabase) : this(db.trackerDao())
     ...
     * Port of TrackerStore.insert(_:deviceId:).
     */
    ...
    /** All cached daily metrics for a device, oldest first. Feeds net.wilsonschwegler.warbfit.analytics.IllnessWatch. */
    suspend fun days(deviceId: String): List<DailyMetric> = dao.days(deviceId)

    // MARK: - Merged reads (imported source wins per day; computed "-warbfit" gap-fills)
    //
    // Mirrors macOS Repository.mergeDaily / mergeSleep: the IntelligenceEngine persists
    // on-device scores under "<deviceId>-warbfit"; the dashboard should see BOTH sources so
    // a strap-only user still gets a populated dashboard, while a real TRACKER import always
    // wins on the days it covers. The screens point their "my-tracker" reads at these merged
    // variants (the least invasive correct approach — no DAO/schema change, and the per-day
    // precedence lives in one place).

    /** The computed-source id for a given imported [deviceId] (e.g. "my-tracker" → "my-tracker-warbfit"). */
    fun computedDeviceId(deviceId: String): String = "$deviceId-warbfit"

    /**
     * All cached daily metrics for [deviceId], oldest first, MERGED with the on-device
     * computed scores from "<deviceId>-warbfit". Imported rows win per day; computed rows
     ...
     * computed "<deviceId>-warbfit" rows gap-fill. Emits whenever either source changes.
     */
     ...
     * "<deviceId>-warbfit" sessions. Imported sessions win per night-end day; computed sessions
    ...
    companion object {
        ...
        fun from(context: Context): TrackerRepository = TrackerRepository(TrackerDatabase.get(context))
        ...
        internal fun mergeDaily(
            ...
            for (d in imported) byDay[d.day] = d // …import overwrites, so a real TRACKER import always wins
        ...
        internal fun mergeSleep(
            ...
            fun endDay(s: SleepSession): String =
                net.wilsonschwegler.warbfit.analytics.AnalyticsEngine.dayString(s.endTs)
