...
/// Which Tracker hardware generation a connection / capture belongs to.
///
...
public enum DeviceFamily: String, Sendable, CaseIterable {
    /// Tracker 4.0 — 0x07 CRC8 header check.
    case tracker4
    /// Tracker 5.0 / MG — CRC16-Modbus header check, "puffin" packet types.
    case tracker5
}

/// Which checksum guards the frame header for a given device family.
public enum HeaderCRCKind: String, Sendable, CaseIterable {
    /// CRC8 (poly 0x07) over the two declared-length bytes — Tracker 4.0.
    case crc8
    /// CRC16-Modbus (poly 0xA001, init 0xFFFF, reflected) over the header prefix — Tracker 5.0.
    case crc16Modbus
}
...
public extension DeviceFamily {
    ...
    var headerCRCKind: HeaderCRCKind {
        switch self {
        case .tracker4: return .crc8
        case .tracker5: return .crc16Modbus
        }
    }
    ...
    var serviceUUIDString: String {
        switch self {
        case .tracker4: return "61080001-8d6d-82b8-614a-1c8cb0f8dcc6"
        case .tracker5: return "fd4b0001-cce1-4033-93ce-002d5875f58a"
        }
    }

    /// Characteristic UUID *strings* this family uses, in stable ascending order.
    ///
    /// Tracker 4.0 exposes 0002…0005 under the 6108 service. Tracker 5.0 adds an extra 0007
    ...
    var commandCharacteristicUUIDString: String {
        switch self {
        case .tracker4: return "61080002-8d6d-82b8-614a-1c8cb0f8dcc6"
        case .tracker5: return "fd4b0002-cce1-4033-93ce-002d5875f58a"
        }
    }
    ...
    ///
    /// The Tracker 5.0 hello is a fully-formed type-35 (COMMAND) frame with CRC16-Modbus header and
    ...
    var clientHello: [UInt8]? {
        ...
        case .tracker4:
            return nil
        case .tracker5:
            return DeviceFamily.tracker5ClientHello
        }
    }

    /// Tracker 5.0 CLIENT_HELLO bytes (16 bytes). Exposed as a named constant for test/debug use.
    static let tracker5ClientHello: [UInt8] = [
...
/// Tracker 5.0 introduces "puffin" packet types that mirror existing 4.0 types but on the new
...
/// Canonical type name for a packet type byte, aliasing the Tracker 5.0 "puffin" types onto the
