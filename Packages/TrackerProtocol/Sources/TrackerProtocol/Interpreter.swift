...
///
/// `tracker4` behaves EXACTLY like the no-family `parseFrame(_:)` above (back-compat). `tracker5`
/// parses the Tracker 5.0 envelope (see `verifyFrame(_:family:)` for the layout): the SOF/length/
...
public func parseFrame(_ frame: [UInt8], family: DeviceFamily) -> ParsedFrame {
    ...
    case .tracker5:
        return parseFrameTracker5(frame)
    }
}

private func parseFrameTracker5(_ frame: [UInt8]) -> ParsedFrame {
    let rawHex = frame.map { String(format: "%02x", $0) }.joined()
    ...
    // Tracker 5.0 biometric field offsets are a later milestone — for now expose the inner record as
