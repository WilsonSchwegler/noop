...
/// CRC16-Modbus (poly 0xA001, init 0xFFFF, reflected). Used for the Tracker 5.0 frame header check.
...
///
/// `tracker4` behaves EXACTLY like the no-family `verifyFrame(_:)` above (back-compat). `tracker5`
/// uses the Tracker 5.0 envelope reverse-engineered from Goose:
///
///   [0]   SOF 0xAA
///   [1]   format byte (0x01)
...
///
/// For tracker5 the `crc8OK` field of the result carries the CRC16 header outcome (so callers get a
...
public func verifyFrame(_ frame: [UInt8], family: DeviceFamily) -> FrameCheck {
    ...
    case .tracker5:
        return verifyFrameTracker5(frame)
    }
}

private func verifyFrameTracker5(_ frame: [UInt8]) -> FrameCheck {
    // Minimum tracker5 frame: 8 header bytes (incl. CRC16) + 4 CRC32 trailer = 12.
...
/// (the static CLIENT_HELLO). The returned frame round-trips through `verifyFrame(_:family:.tracker5)`.
...
public final class Reassembler {
    ...
    /// - TRACKER 4.0 reads a u16 length at `buf[1..3]`, total = `length + 4`.
    /// - TRACKER 5.0 ("puffin") reads a u16 declared length at `buf[2..4]` (after the `0xAA` SOF and
    ...
    ///
    /// Defaults to `.tracker4` so existing callers and tests are byte-for-byte unchanged.
    public init(family: DeviceFamily = .tracker4) {
        self.family = family
    }
