...
public func loadSchema() -> Schema {
    ...
    guard let url = Bundle.module.url(forResource: "tracker_protocol", withExtension: "json") else {
        fatalError("tracker_protocol.json missing from Bundle.module resources")
    }
    let data: Data
    do {
        data = try Data(contentsOf: url)
    } catch {
        fatalError("failed to read tracker_protocol.json: \(error)")
    }
    let raw: RawSchema
    do {
        raw = try JSONDecoder().decode(RawSchema.self, from: data)
    } catch {
        fatalError("failed to decode tracker_protocol.json: \(error)")
    }
