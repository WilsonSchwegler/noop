...
final class DeviceFamilyFramingTests: XCTestCase {
    ...
    // MARK: - Tracker 5.0 frame verification

    // Synthetic, hand-computed VALID tracker5 frame:
    ...
    static let validTracker5 = "aa010c000001e74124070211223344557481f36e"

    func testVerifyValidTracker5Frame() {
        let frame = Self.hex(Self.validTracker5)
        let check = verifyFrame(frame, family: .tracker5)
    ...
    func testTracker4OnTracker5FrameFailsHeaderCRC() {
        // Same bytes, parsed as tracker4: the CRC8 path reads length/crc8 from the wrong offsets,
        // so the header CRC must fail — proving the family switch genuinely changes behaviour.
        let frame = Self.hex(Self.validTracker5)
        let check = verifyFrame(frame, family: .tracker4)
        XCTAssertEqual(check.crc8OK, false)
        XCTAssertFalse(check.ok)
    }

    func testTracker5HeaderCorruptionDetected() {
        var frame = Self.hex(Self.validTracker5)
    ...
    func testTracker5PayloadCorruptionDetected() {
        var frame = Self.hex(Self.validTracker5)
    ...
    func testTracker5ShortFrameRejected() {
        XCTAssertFalse(verifyFrame([0xAA, 0x01, 0x0c, 0x00], family: .tracker5).ok)
    }

    func testTracker5NonSOFRejected() {
        var frame = Self.hex(Self.validTracker5)
        frame[0] = 0x00
        XCTAssertFalse(verifyFrame(frame, family: .tracker5).ok)
    }

    // MARK: - tracker4 family overload is byte-for-byte back-compatible

    func testTracker4FamilyOverloadMatchesLegacy() {
        ...
        XCTAssertEqual(verifyFrame(frame, family: .tracker4), verifyFrame(frame))
        XCTAssertTrue(verifyFrame(frame, family: .tracker4).ok)
    }

    // MARK: - CLIENT_HELLO constant

    func testTracker5ClientHelloBytesAndVerify() {
        let hello = DeviceFamily.tracker5ClientHello
        XCTAssertEqual(hello, Self.hex("aa0108000001e67123019101363e5c8d"))
        XCTAssertEqual(DeviceFamily.tracker5.clientHello, hello)
        XCTAssertNil(DeviceFamily.tracker4.clientHello)
        // The real CLIENT_HELLO must pass tracker5 verification end-to-end.
        let check = verifyFrame(hello, family: .tracker5)
        XCTAssertTrue(check.ok)
    ...
    func testFamilyMetadata() {
        XCTAssertEqual(DeviceFamily.tracker4.headerCRCKind, .crc8)
        XCTAssertEqual(DeviceFamily.tracker5.headerCRCKind, .crc16Modbus)

        XCTAssertEqual(DeviceFamily.tracker4.serviceUUIDString,
                       "61080001-8d6d-82b8-614a-1c8cb0f8dcc6")
        XCTAssertEqual(DeviceFamily.tracker5.serviceUUIDString,
                       "fd4b0001-cce1-4033-93ce-002d5875f58a")

        XCTAssertEqual(DeviceFamily.tracker4.characteristicUUIDStrings.count, 4)
        XCTAssertEqual(DeviceFamily.tracker5.characteristicUUIDStrings.count, 5)
        XCTAssertTrue(DeviceFamily.tracker5.characteristicUUIDStrings
            .contains("fd4b0007-cce1-4033-93ce-002d5875f58a"))

        XCTAssertEqual(DeviceFamily.tracker4.commandCharacteristicUUIDString,
                       "61080002-8d6d-82b8-614a-1c8cb0f8dcc6")
        XCTAssertEqual(DeviceFamily.tracker5.commandCharacteristicUUIDString,
                       "fd4b0002-cce1-4033-93ce-002d5875f58a")

        XCTAssertEqual(DeviceFamily.allCases, [.tracker4, .tracker5])
    }
    ...
    func testParseTracker5FrameAliasesPuffinAndParsesEnvelope() {
        ...
        let parsed = parseFrame(frame, family: .tracker5)
    ...
    func testParseTracker5CommandResponseFrame() {
        let frame = Self.hex(Self.validTracker5)
        let parsed = parseFrame(frame, family: .tracker5)
        XCTAssertTrue(parsed.ok)
    ...
    func testParseTracker4FamilyOverloadMatchesLegacy() {
        let frame = Self.hex("aa1800ff28020f3de10128663c0000000000000000000000da855212")
        XCTAssertEqual(parseFrame(frame, family: .tracker4), parseFrame(frame))
    }

    // MARK: - Family-aware Reassembler

    func testReassemblerTracker5SingleFrame() {
        // validTracker5 is a complete 20-byte frame: declLength=12 @ [2..4] -> total = 12 + 8 = 20.
        let frame = Self.hex(Self.validTracker5)
        let r = Reassembler(family: .tracker5)
        let out = r.feed(frame)
        XCTAssertEqual(out.count, 1)
        XCTAssertEqual(out.first, frame)
    }

    func testReassemblerTracker5SplitAcrossFragments() {
        // A tracker5 frame split mid-way must only emit once both halves have arrived.
        let frame = Self.hex(Self.validTracker5)
        let r = Reassembler(family: .tracker5)
    ...
    func testReassemblerTracker5BackToBackFrames() {
        // Two tracker5 frames in one buffer (the 16-byte CLIENT_HELLO then the 20-byte fixture).
        let hello = DeviceFamily.tracker5ClientHello     // declLength=8 -> total 16
        let frame = Self.hex(Self.validTracker5)          // total 20
        let r = Reassembler(family: .tracker5)
    ...
    func testReassemblerTracker5DiscardsLeadingGarbage() {
        let frame = Self.hex(Self.validTracker5)
        let r = Reassembler(family: .tracker5)
    ...
    func testPuffinCommandFrameVerifies() {
        // A puffin TOGGLE_REALTIME_HR (cmd 3, payload [0x01]) must be a well-formed tracker5 frame.
        let f = puffinCommandFrame(cmd: 3, seq: 7, payload: [0x01])
        let check = verifyFrame(f, family: .tracker5)
        XCTAssertTrue(check.ok)
        XCTAssertEqual(check.crc8OK, true)    // CRC16 header outcome surfaced via crc8OK
        XCTAssertEqual(check.crc32OK, true)
        // And it parses back as a tracker5 frame with the seq we set.
        let parsed = parseFrame(f, family: .tracker5)
        XCTAssertTrue(parsed.ok)
        XCTAssertEqual(parsed.seq, 7)
        // It also reassembles cleanly through the tracker5 reassembler.
        XCTAssertEqual(Reassembler(family: .tracker5).feed(f), [f])
    }

    func testReassemblerTracker4DefaultUnchanged() {
        // Default family stays TRACKER 4.0: a 28-byte tracker4 frame (length=0x18=24 -> total 28).
        let frame = Self.hex("aa1800ff28020f3de10128663c0000000000000000000000da855212")
        XCTAssertEqual(Reassembler().feed(frame), [frame])
        XCTAssertEqual(Reassembler(family: .tracker4).feed(frame), [frame])
    }
}
