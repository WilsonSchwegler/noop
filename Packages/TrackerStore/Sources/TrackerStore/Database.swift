...
extension TrackerStore {
    ...
    static func makeMigrator() -> DatabaseMigrator {
        ...
        migrator.registerMigration("v10") { db in
            // Minute-resolution heart-rate archive for finalized daily views. Fine-grain sensor
            // rows can be pruned after the day is finalized, while the UI still has the chart data
            // it needs for historical Today/Sleep/HR views.
            try db.create(table: "dailyHeartRateArchive") { t in
                t.column("deviceId", .text).notNull()
                t.column("day", .text).notNull()
                t.column("ts", .integer).notNull()
                t.column("bpm", .integer).notNull()
                t.primaryKey(["deviceId", "day", "ts"])
            }
            try db.create(index: "idx_dailyHeartRateArchive_device_day_ts",
                          on: "dailyHeartRateArchive", columns: ["deviceId", "day", "ts"])
        }
        return migrator
    }
}
