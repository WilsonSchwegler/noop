...
extension TrackerStore {
    ...
    public func enqueueRawBatch(_ meta: RawBatchMeta, frames: [[UInt8]]) async throws {
        let packed = TrackerStore.packFrames(frames)
        let blob = try TrackerStore.zlibCompressWithLength(packed)
        try syncWrite { db in
            try db.execute(sql: """
    ...
    public func rawFrames(batchId: String) async throws -> [[UInt8]] {
        ...
        let raw = try TrackerStore.zlibDecompressWithLength(blob)
        return TrackerStore.unpackFrames(raw)
    }
    ...
    public func pendingRawBatches(limit: Int) async throws -> [RawBatchMeta] {
        try syncRead { db in
            try Row.fetchAll(db, sql: """
                ...
                LIMIT ?
                """, arguments: [limit]).map(TrackerStore.metaFromRow)
        }
    }
