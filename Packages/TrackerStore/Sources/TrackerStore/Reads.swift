...
extension TrackerStore {
    ...
    public func events(deviceId: String, from: Int, to: Int, limit: Int) async throws -> [TrackerEvent] {
        try syncRead { db in
            try Row.fetchAll(db, sql: """
                ...
                .map { row in
                    ...
                    let payload = (try? TrackerStore.eventDecoder.decode(
                        [String: ParsedValue].self,
                        from: Data(json.utf8))) ?? [:]
                    return TrackerEvent(ts: row["ts"], kind: row["kind"], payload: payload)
                }
        }
    }
