...
public enum TrackerStoreInfo {
    /// Bumped whenever the migrator gains a new migration.
    public static let schemaVersion = 10
}

/// TrackerStore is an `actor`: its public API is `async`, and all GRDB work runs on the
...
public actor TrackerStore {
    ...
    private init(dbQueue: DatabaseQueue) throws {
        ...
        try TrackerStore.makeMigrator().migrate(dbQueue)
    }
    ...
    public static func inMemory() async throws -> TrackerStore {
        try TrackerStore(dbQueue: try DatabaseQueue())
    }
    ...
    /// Used before a file-level backup so the single `tracker.sqlite` carries all committed data
    ...
    /// Reclaim disk space after large retention deletes. This is intentionally explicit because
    /// VACUUM can be expensive; callers should only run it after meaningful pruning.
    public func compactStorage() async throws {
        try compactStorageImpl()
    }
    ...
    private func compactStorageImpl() throws {
        try dbQueue.writeWithoutTransaction { db in
            try db.execute(sql: "PRAGMA wal_checkpoint(TRUNCATE)")
