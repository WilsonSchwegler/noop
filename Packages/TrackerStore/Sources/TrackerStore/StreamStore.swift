...
extension TrackerStore {
    ...
    @discardableResult
    public func mergeDeviceData(from sourceID: String, into destinationID: String) async throws -> Int {
        guard sourceID != destinationID else { return 0 }
        return try syncWrite { db in
            var changed = 0
            for table in Self.deviceScopedTables {
                try db.execute(
                    sql: "UPDATE OR IGNORE \(table) SET deviceId = ? WHERE deviceId = ?",
                    arguments: [destinationID, sourceID]
                )
                changed += db.changesCount
                try db.execute(sql: "DELETE FROM \(table) WHERE deviceId = ?", arguments: [sourceID])
                changed += db.changesCount
            }
            try db.execute(sql: "DELETE FROM device WHERE id = ?", arguments: [sourceID])
            changed += db.changesCount
            return changed
        }
    }

    private static let deviceScopedTables = [
        "hrSample",
        "rrInterval",
        "event",
        "battery",
        ...
        return try syncWrite { db in
            ...
            if !streams.events.isEmpty {
                ...
                for e in streams.events {
                    let json = try TrackerStore.encodePayload(e.payload)
