...
struct IOSLoadGuidance: Equatable {
    ...
    var detail = "Collect more fitness tracker exertion history for training-load guidance."
    var acwr: Double?
...
struct IOSTrackerDeviceSnapshot: Equatable {
    ...
    var restingHR: Int?
    var exertionRestingHR: Double?
    var hrvRMSSD: Double?
    var sleepHRVRMSSD: Double?
    var trackerSleepHours = 0.0
    var trackerSleepEfficiency = 0.0
    var trackerSleepStages: [IOSSleepStageSummary] = []
    var trackerSleepIntervals: [IOSSleepInterval] = []
    var trackerSleepStatus = "Waiting for overnight fitness tracker HR"
    ...
    static let empty = IOSTrackerDeviceSnapshot()
}

enum IOSTrackerDeviceMetrics {
    static let dailyLogVersion = 11
    private static let maxLoggedSleepIntervals = 96

    static func loggedDaySnapshot(store: TrackerStore,
                                  deviceId: String,
                                  date: Date,
                                  calendar: Calendar = .current) async throws -> IOSTrackerDeviceSnapshot? {
        ...
        var hr = try await store.archivedHeartRateSamples(deviceId: deviceId, day: selectedDay, from: selectedStart, to: selectedEnd, limit: 2_000)
        if hr.isEmpty {
            hr = try await store.hrSamples(deviceId: deviceId, from: selectedStart, to: selectedEnd, limit: 120_000)
        }
        var contextHR = try await store.archivedHeartRateSamples(deviceId: deviceId, day: selectedDay, from: selectedStart - 12 * 3600, to: selectedEnd, limit: 3_000)
        if contextHR.isEmpty {
            contextHR = try await store.hrSamples(deviceId: deviceId, from: selectedStart - 12 * 3600, to: selectedEnd, limit: 160_000)
        }
        ...
        if !gravity.isEmpty || values["warbfit.activityPoints"] == nil {
            snapshot.activityPoints = gravity.count
        }
        return snapshot
    }

    static func refresh(store: TrackerStore,
                        deviceId: String,
                        now: Date = Date(),
                        calendar: Calendar = .current,
                        userProfile: IOSUserBodyProfile = .current,
                        recalculateSleep: Bool = false) async throws -> IOSTrackerDeviceSnapshot {
        ...
        if !selectedDayIsToday, hasCurrentLoggedMetrics, loggedMetrics["warbfit.finalized"] == 1 {
            var historicalHR = try await store.archivedHeartRateSamples(deviceId: deviceId, day: selectedDay, from: todayStart, to: selectedEnd, limit: 2_000)
            if historicalHR.isEmpty {
                historicalHR = todayHR
            }
            ...
            if !todayGravity.isEmpty || loggedMetrics["warbfit.activityPoints"] == nil {
        ...
        let maxHR = StrainScorer
            .estimateHRmax(historyHR.map { Double($0.bpm) }, age: userProfile.ageForHRMax)
            .0
            .nonZero
        ...
        if !recalculateSleep, let cachedSleep {
            ...
            sleepSummary = trackerSleepSummary(
                hr: sleepHR,
                rr: sleepRR,
                resp: sleepResp,
                gravity: sleepGravity,
        ...
        let exertionRestingHR = IOSStrainEstimator.exertionRestingHR(
            baseline: recoveryParts.exertionRestingHR,
            recentRestingHR: recoveryParts.restingHR.map(Double.init),
            samples: awakeHR,
            source: .tracker
        )
        IOSStrainEstimator.saveExertionRestingHRBaseline(recoveryParts.exertionRestingHR, for: .tracker)
        let strain = dailyStrainValue(
            awakeHR,
            maxHR: maxHR,
            restingHR: exertionRestingHR,
            physiologySex: userProfile.physiologySex.analyticsValue,
        ...
        let detected = WorkoutDetector.detect(
            ...
            restingHR: exertionRestingHR,
            age: userProfile.ageForHRMax,
            profile: userProfile.analyticsProfile
        )
        ...
        var snapshot = IOSTrackerDeviceSnapshot()
        ...
        snapshot.exertionRestingHR = exertionRestingHR
        snapshot.hrvRMSSD = recoveryParts.hrv
        snapshot.trackerSleepHours = sleepSummary.hours
        snapshot.trackerSleepEfficiency = sleepSummary.efficiency
        snapshot.trackerSleepStages = sleepSummary.stages
        snapshot.trackerSleepIntervals = sleepSummary.intervals
        snapshot.trackerSleepStatus = sleepSummary.status
    ...
    private static func loggedMetrics(store: TrackerStore, deviceId: String, day: String) async throws -> [String: Double] {
        var keys = [
            "warbfit.logVersion",
    ...
    private static func loggedSnapshot(values: [String: Double],
                                       ...
                                       latest: Int?,
                                       hr: [HRSample]) -> IOSTrackerDeviceSnapshot {
        var snapshot = IOSTrackerDeviceSnapshot()
        snapshot.decodedRows = stats.decodedRows
        snapshot.latestSampleAt = latest.map { Date(timeIntervalSince1970: TimeInterval($0)) }
    ...
    private static func applyLoggedMetrics(_ values: [String: Double], to snapshot: inout IOSTrackerDeviceSnapshot) {
        if let strain = values["strain"] { snapshot.strain = strain }
        if let recovery = values["recovery"] { snapshot.recovery = recovery }
        if let sleepHours = values["warbfit.sleepHours"] { snapshot.trackerSleepHours = sleepHours }
        if let sleepEfficiency = values["warbfit.sleepEfficiency"] { snapshot.trackerSleepEfficiency = sleepEfficiency }
        ...
        if let exertionRestingHR = values["warbfit.exertionRestingHR"] { snapshot.exertionRestingHR = exertionRestingHR }
    ...
    private static func applyLoggedSleep(_ values: [String: Double], to snapshot: inout IOSTrackerDeviceSnapshot) {
        ...
        snapshot.trackerSleepStages = stagePairs.compactMap { name, key in
        ...
        if let storedIntervals = loggedSleepIntervals(from: values), !storedIntervals.isEmpty {
            snapshot.trackerSleepIntervals = storedIntervals
            if snapshot.trackerSleepStages.isEmpty {
                snapshot.trackerSleepStages = summarize(intervals: storedIntervals).stages
            }
            snapshot.trackerSleepStatus = "Logged sleep snapshot"
            return
        }
        guard let start = values["warbfit.sleepStartTs"],
              let end = values["warbfit.sleepEndTs"],
              end > start else { return }
        if snapshot.trackerSleepStages.isEmpty, snapshot.trackerSleepHours > 0 {
            snapshot.trackerSleepStages = [IOSSleepStageSummary(name: "Core", hours: snapshot.trackerSleepHours)]
        }
        snapshot.trackerSleepIntervals = snapshot.trackerSleepStages.reduce(into: []) { intervals, stage in
    ...
    private static func loggedSleepSummary(from values: [String: Double]) -> (hours: Double, efficiency: Double, stages: [IOSSleepStageSummary], intervals: [IOSSleepInterval], status: String)? {
        ...
        var snapshot = IOSTrackerDeviceSnapshot()
        snapshot.trackerSleepHours = max(0, values["warbfit.sleepHours"] ?? 0)
        snapshot.trackerSleepEfficiency = max(0, min(1, values["warbfit.sleepEfficiency"] ?? 0))
        applyLoggedSleep(values, to: &snapshot)
        guard !snapshot.trackerSleepIntervals.isEmpty else { return nil }
        let hours = snapshot.trackerSleepHours > 0
            ? snapshot.trackerSleepHours
            : snapshot.trackerSleepIntervals
                .filter { $0.stage != "Awake" }
                .reduce(0.0) { $0 + $1.end.timeIntervalSince($1.start) / 3600.0 }
        let efficiency = snapshot.trackerSleepEfficiency > 0
            ? snapshot.trackerSleepEfficiency
            : intervalSleepEfficiency(snapshot.trackerSleepIntervals) ?? 0
        let status = (values["warbfit.sleepManualUpdatedAt"] ?? 0) > 0
            ? "Adjusted sleep window"
            : "Logged sleep snapshot"
        return (hours, efficiency, snapshot.trackerSleepStages, snapshot.trackerSleepIntervals, status)
    }
    ...
    private static func manualSleepSummary(start: Int,
        ...
        return (
            summary.hours,
            staged.efficiency ?? intervalSleepEfficiency(safeIntervals) ?? 0,
            summary.stages,
            safeIntervals,
            staged.status
        )
    ...
    private static func loggedRecoveryParts(from values: [String: Double]) -> (score: Double?, status: String, restingHR: Int?, hrv: Double?, spo2RawRatio: Double?, skinTempRaw: Double?, exertionRestingHR: Double?)? {
        ...
        let exertionRestingHR = values["warbfit.exertionRestingHR"]
        return (
            recovery,
            "Logged readiness snapshot from saved sleep window",
            restingHR,
            hrv,
            spo2,
            skinTemp,
    ...
    static func trackerSleepSummary(hr: [HRSample],
                                  rr: [RRInterval],
                                  resp: [RespSample],
    ...
    private static func stageKnownSleepWindow(start: Int,
                                              ...
                                              resp: [RespSample],
                                              gravity: [GravitySample],
                                              statusPrefix: String) -> (intervals: [IOSSleepInterval], efficiency: Double?, status: String) {
        ...
        if hasActigraphyCoverage {
            ...
            if !summary.intervals.isEmpty {
                return (summary.intervals, summary.efficiency, summary.status)
            }
        }
        ...
        return (intervals, nil, "\(statusPrefix) estimated from fitness tracker \(inputText); stage confidence is lower without motion coverage")
    }
    ...
    private static func intervalSleepEfficiency(_ intervals: [IOSSleepInterval]) -> Double? {
        let valid = intervals
            .filter { $0.end > $0.start }
            .sorted { $0.start < $1.start }
    ...
    private static func recovery(historyHR: [HRSample],
                                 ...
                                 userProfile: IOSUserBodyProfile,
                                 calendar: Calendar) -> (score: Double?, status: String, restingHR: Int?, hrv: Double?, spo2RawRatio: Double?, skinTempRaw: Double?, exertionRestingHR: Double?) {
        ...
        let currentSession = sessions
            ...
            .max { $0.end < $1.end }
        let trackerSegments = healthSleepSegments(from: sleepSummary.intervals)
        let currentSegments = trackerSegments.isEmpty ? (currentSession.map(asleepSegments) ?? []) : trackerSegments
        ...
        let exertionRestingHRBaseline = rhrBaseline.usable ? rhrBaseline.baseline : nil
        ...
        let loadExertionRestingHR = IOSStrainEstimator.exertionRestingHR(
            baseline: exertionRestingHRBaseline,
            recentRestingHR: rhrNights.compactMap { $0 }.last,
            samples: historyHR,
            source: .tracker
        )
        let loadRatio = acuteChronicStrainRatio(
            ...
            selectedDayStart: todayStart,
            loggedStrainByDay: loggedStrainByDay,
            exertionRestingHR: loadExertionRestingHR,
            userProfile: userProfile,
            calendar: calendar
        )
        ...
        guard let currentHRV, let currentRHR else {
            return (
                nil,
                "Waiting for overnight fitness tracker R-R and HR data",
                currentRHR,
                currentHRV,
                currentSpO2Raw,
                currentSkinRaw,
                exertionRestingHRBaseline
            )
        }
        ...
        guard let score else {
            ...
            return (
                provisional,
                "Provisional WarbFit readiness while fitness tracker baseline calibrates \(hrvBaseline.nValid)/\(Baselines.minNightsSeed) nights",
                currentRHR,
                currentHRV,
                currentSpO2Raw,
                currentSkinRaw,
        ...
        return (
            score,
            "WarbFit readiness from fitness tracker sleep, HRV/RHR\(respText)\(loadText)\(illnessText)\(suffix)",
            currentRHR,
            currentHRV,
            currentSpO2Raw,
            currentSkinRaw,
    ...
    private static func sleepPerformance(summary: (hours: Double, efficiency: Double, stages: [IOSSleepStageSummary], intervals: [IOSSleepInterval], status: String)) -> Double? {
        ...
        if summary.efficiency > 0 {
            weighted += min(1.0, max(0.0, summary.efficiency) / 0.85) * 0.25
            weight += 0.25
        }
    ...
    private static func acuteChronicStrainRatio(historyHR: [HRSample],
        ...
        for offset in stride(from: 28, through: 1, by: -1) {
            ...
            strains.append(dailyStrainValue(
                dayHR,
                maxHR: maxHR,
                restingHR: exertionRestingHR,
                physiologySex: userProfile.physiologySex.analyticsValue,
    ...
    private static func loadGuidance(historyHR: [HRSample],
        ...
        for offset in stride(from: 28, through: 0, by: -1) {
            if offset == 0, let selectedDayStrain {
                dayStrains.append(selectedDayStrain)
                continue
            }
            let start = selectedDayStart - offset * 24 * 3600
            let end = start + 24 * 3600 - 1
    ...
    private static func strainValue(_ hr: [HRSample],
                                    ...
                                    maxHR: Double?,
                                    restingHR: Double,
                                    physiologySex: String = "nonbinary",
