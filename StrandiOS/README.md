# WarbFit iOS Development Target
...
- Uses the WarbFit app icon artwork.
...
- Scans for the fitness tracker 4.0 custom BLE service.
...
- Performs a benign confirmed `GET_BATTERY_LEVEL` write to trigger fitness
  tracker 4.0 just-works bonding.
- Runs the fitness tracker 4.0 connect handshake, sets/reads clock, and requests historical
  offload into the local SQLite store.
- Persists fitness tracker HR, R-R, battery, and decoded historical sensor streams locally.
- Calculates strain, workout detection, HRV, resting HR, and recovery from the
  fitness tracker store. Apple Health is used only for sleep analysis/sleep score.
- Can send a safe haptic buzz command and arm the strap alarm once the command
  channel is ready.

## What is not ported yet

- fitness tracker 5.0 / MG custom protocol support.
- Widgets, App Intents, and import screens.

## Run on an iPhone

1. Install full Xcode, not just Command Line Tools.
2. Run `xcodegen generate` from the repo root.
3. Open `WarbFit.xcodeproj`.
4. Select the `WarbFitiOS` scheme.
...
For first pairing, keep the official fitness tracker app closed or out of range so the strap
is available to WarbFit.
