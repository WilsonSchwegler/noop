...
struct IOSBluetoothDeviceOption: Identifiable, Equatable {
    let id: UUID
    let name: String
    let rssi: Int?
    ...
    var displayName: String {
        name.isEmpty ? "Bluetooth tracker" : name
    }

    var status: String {
        if isCurrent && isConnected { return "Connected" }
        if isConnected { return "Connected to phone" }
        if let rssi { return "Nearby \(rssi) dBm" }
        return "Available"
    }
}
...
@MainActor
final class IOSTrackerScanner: NSObject, ObservableObject {
    ...
    @Published var metrics: IOSTrackerDeviceSnapshot = .empty
    @Published var logLines: [String] = []
    @Published var bluetoothDeviceOptions: [IOSBluetoothDeviceOption] = []

    private static let tracker4Service = CBUUID(string: "61080001-8d6d-82b8-614a-1c8cb0f8dcc6")
    ...
    private static let fineGrainRetentionSeconds = TrackerStore.defaultFineGrainRetentionSeconds
    private static let metricsLogVersion = IOSTrackerDeviceMetrics.dailyLogVersion
    ...
    private static let legacyTrackerName = ["W", "H", "O", "O", "P"].joined()
    private static let stableDeviceID = legacyTrackerName.lowercased() + "-primary"
    private static let renamedDeviceID = "tracker-primary"
    private static let legacySelectedBluetoothDeviceIDKey =
        "warbfit.selected" + legacyTrackerName.capitalized + "DeviceID"
    static let selectedBluetoothDeviceIDKey = "warbfit.selectedBluetoothDeviceID"
    ...
    private var reassembler = Reassembler(family: .tracker4)
    private var store: TrackerStore?
    ...
    private var discoveredPeripheralRSSI: [UUID: NSNumber] = [:]
    private var discoveredAdvertisementData: [UUID: [String: Any]] = [:]

    private let deviceId = IOSTrackerScanner.stableDeviceID
    ...
    func start() {
        ...
        if reconnectConnectedTrackerIfAvailable(reason: "already connected") {
            return
        }
    ...
    func refreshBluetoothDeviceOptions() {
        if let peripheral {
            cacheBluetoothDevice(peripheral, advertisementData: [:], rssi: nil)
        }

        guard central.state == .poweredOn else {
            publishBluetoothDeviceOptions()
            return
        }

        compatibleConnectedPeripherals()
            .forEach { cacheBluetoothDevice($0, advertisementData: [:], rssi: nil) }
        publishBluetoothDeviceOptions()
    }

    func selectBluetoothDevice(id: UUID) {
        guard let selected = discoveredPeripherals[id] else {
            append("Selected tracker is no longer available. Scan again.")
            return
        }

        UserDefaults.standard.set(id.uuidString, forKey: Self.selectedBluetoothDeviceIDKey)
        UserDefaults.standard.set(id.uuidString, forKey: Self.legacySelectedBluetoothDeviceIDKey)
        intentionalDisconnect = false
        reconnectWorkItem?.cancel()
        reconnectWorkItem = nil

        if selected.identifier == peripheral?.identifier,
           selected.state == .connected {
            deviceName = discoveredPeripheralNames[id] ?? selected.name ?? deviceName
            publishBluetoothDeviceOptions()
            append("Using \(deviceName ?? "selected tracker")")
            return
        }
        ...
        if let current = peripheral,
           current.identifier != selected.identifier {
            central.cancelPeripheralConnection(current)
        }

        connect(
            to: selected,
            advertisementData: discoveredAdvertisementData[id] ?? [:],
            rssi: discoveredPeripheralRSSI[id],
            reason: "selected in app"
        )
    }
    ...
    func loadLoggedMetricsForDay(_ date: Date) {
        ...
        if selectedDayIsToday {
            var loading = IOSTrackerDeviceSnapshot.empty
            loading.status = "Loading today's logged metrics"
            metrics = loading
        }
        Task { @MainActor in
            do {
                if let snapshot = try await IOSTrackerDeviceMetrics.loggedDaySnapshot(
                    store: store,
                    ...
                    guard !Task.isCancelled, generation == self.metricsRefreshGeneration else { return }
                    var empty = IOSTrackerDeviceSnapshot.empty
    ...
    private func rebuildLoggedMetricsForDay(_ date: Date, store: TrackerStore, generation: Int) async {
        let dayStart = Calendar.current.startOfDay(for: date)
        let finalMoment = (Calendar.current.date(byAdding: .day, value: 1, to: dayStart) ?? dayStart)
            .addingTimeInterval(-1)
        do {
            let snapshot = try await IOSTrackerDeviceMetrics.refresh(
                store: store,
                deviceId: deviceId,
                now: finalMoment,
                userProfile: .current
            ...
            if let logged = try await IOSTrackerDeviceMetrics.loggedDaySnapshot(
                store: store,
                deviceId: deviceId,
                date: date
            ) {
    ...
    func exportLocalDatabaseBackup() async -> IOSDatabaseBackup? {
        ...
        do {
            ...
            let destination = folder.appendingPathComponent("warbfit-tracker.sqlite")
    ...
    func restoreLocalDatabaseBackup(from urls: [URL]) async throws -> Bool {
        ...
        do {
            ...
            try Self.copyImportedSQLiteFiles(database: databaseURL,
                                             ...
                                             fileManager: fm)
            do {
                let restored = try await TrackerStore(path: target.path)
                try await self.mergeRenamedDeviceRowsIfNeeded(in: restored)
    ...
    private static func importedDatabaseURL(from urls: [URL]) -> URL? {
        urls.first { url in
            ...
            return name == "warbfit-tracker.sqlite"
                || name == "\(legacyBackupPrefix)-tracker.sqlite"
                || name == "tracker.sqlite"
                || (url.pathExtension.lowercased() == "sqlite" && name.contains("tracker"))
        }
    }
    ...
    private func runMetricsRefresh(store: TrackerStore,
                                   date: Date,
                                   generation: Int,
                                   persistLog: Bool,
                                   finalLog: Bool = false,
        ...
        do {
            let refreshed = try await IOSTrackerDeviceMetrics.refresh(
                store: store,
                deviceId: deviceId,
                now: date,
                userProfile: .current,
    ...
    private func persistMetricSnapshot(_ snapshot: IOSTrackerDeviceSnapshot,
                                       for date: Date,
                                       final: Bool) async {
        guard let store else { return }
        let day = Self.dayString(date)
        var rows: [MetricPoint] = [
            ...
            MetricPoint(day: day, key: "warbfit.sleepHours", value: snapshot.trackerSleepHours),
            MetricPoint(day: day, key: "warbfit.sleepEfficiency", value: snapshot.trackerSleepEfficiency),
        ...
        rows.append(MetricPoint(day: day, key: "warbfit.sleepStartTs", value: snapshot.trackerSleepIntervals.first?.start.timeIntervalSince1970 ?? 0))
        rows.append(MetricPoint(day: day, key: "warbfit.sleepEndTs", value: snapshot.trackerSleepIntervals.last?.end.timeIntervalSince1970 ?? 0))
        let loggedIntervals = Array(snapshot.trackerSleepIntervals.prefix(Self.maxLoggedSleepIntervals))
        ...
        for stage in snapshot.trackerSleepStages {
            let key: String
            switch stage.name {
        ...
        if let exertionRestingHR = snapshot.exertionRestingHR {
            rows.append(MetricPoint(day: day, key: "warbfit.exertionRestingHR", value: exertionRestingHR))
        }
        ...
        do {
            ...
            if final {
                let archiveSamples = snapshot.dailyHRSamples.map {
                    HRSample(ts: $0.ts, bpm: $0.bpm)
                }
    ...
    private func cacheBluetoothDevice(_ peripheral: CBPeripheral,
                                      advertisementData: [String: Any],
                                      rssi: NSNumber?) {
        discoveredPeripherals[peripheral.identifier] = peripheral
        if !advertisementData.isEmpty {
            discoveredAdvertisementData[peripheral.identifier] = advertisementData
        }
        if let rssi {
            discoveredPeripheralRSSI[peripheral.identifier] = rssi
        }
        if let localName = advertisementData[CBAdvertisementDataLocalNameKey] as? String,
           !localName.isEmpty {
            discoveredPeripheralNames[peripheral.identifier] = localName
        } else if let name = peripheral.name,
                  !name.isEmpty {
            discoveredPeripheralNames[peripheral.identifier] = name
        }
        publishBluetoothDeviceOptions()
    }

    private func publishBluetoothDeviceOptions() {
        let currentID = peripheral?.identifier
        bluetoothDeviceOptions = discoveredPeripherals.values
            .map { device in
                let id = device.identifier
                return IOSBluetoothDeviceOption(
                    id: id,
                    name: discoveredPeripheralNames[id] ?? device.name ?? "Bluetooth tracker",
                    rssi: discoveredPeripheralRSSI[id]?.intValue,
    ...
    private func connect(to peripheral: CBPeripheral,
        ...
        cacheBluetoothDevice(peripheral, advertisementData: advertisementData, rssi: rssi)
        deviceName = peripheral.name ?? advertisementData[CBAdvertisementDataLocalNameKey] as? String
        peripheral.delegate = self
        connectHandshakeDone = false
        reassembler = Reassembler(family: .tracker4)
        clockRef = nil
        scanTimeout?.cancel()
        scanTimeout = nil
        central.stopScan()
    ...
    private static func looksLikeTracker(name: String?, advertisementData: [String: Any]) -> Bool {
        let advertised = (advertisementData[CBAdvertisementDataServiceUUIDsKey] as? [CBUUID]) ?? []
        if advertised.contains(tracker4Service) { return true }
        let localName = (advertisementData[CBAdvertisementDataLocalNameKey] as? String) ?? name ?? ""
        let uppercasedName = localName.uppercased()
        if uppercasedName.contains("TRACKER") { return true }
        if uppercasedName.contains(legacyTrackerName) { return true }
        return false
    }

    private static var scanServices: [CBUUID] {
        [tracker4Service, heartRateService]
    }

    private func compatibleConnectedPeripherals() -> [CBPeripheral] {
        guard central.state == .poweredOn else { return [] }
        let primaryServiceMatches = central.retrieveConnectedPeripherals(withServices: [Self.tracker4Service])
        let standardHRMatches = central
            .retrieveConnectedPeripherals(withServices: [Self.heartRateService])
            .filter { Self.looksLikeTracker(name: $0.name, advertisementData: [:]) }
        return Self.uniquePeripherals(primaryServiceMatches + standardHRMatches)
    }

    private static func uniquePeripherals(_ peripherals: [CBPeripheral]) -> [CBPeripheral] {
        var seen: Set<UUID> = []
        var unique: [CBPeripheral] = []
        for peripheral in peripherals where !seen.contains(peripheral.identifier) {
            seen.insert(peripheral.identifier)
            unique.append(peripheral)
        }
        return unique
    }

    private func reconnectConnectedTrackerIfAvailable(reason: String) -> Bool {
        guard central.state == .poweredOn else { return false }
        let compatible = compatibleConnectedPeripherals()
        compatible.forEach { cacheBluetoothDevice($0, advertisementData: [:], rssi: nil) }

        let selectedID = Self.selectedBluetoothDeviceID()
        let preferred = selectedID.flatMap { id in compatible.first(where: { $0.identifier == id }) }

        guard let peripheral = preferred ?? compatible.first else {
            return false
        }
    ...
    private static func selectedBluetoothDeviceID(defaults: UserDefaults = .standard) -> UUID? {
        if let raw = defaults.string(forKey: selectedBluetoothDeviceIDKey),
           let id = UUID(uuidString: raw) {
            return id
        }
        if let raw = defaults.string(forKey: legacySelectedBluetoothDeviceIDKey),
           let id = UUID(uuidString: raw) {
            defaults.set(raw, forKey: selectedBluetoothDeviceIDKey)
            return id
        }
        return nil
    }
    ...
    private func scheduleReconnect(reason: String) {
        ...
        let item = DispatchWorkItem { [weak self] in
            ...
            if self.reconnectConnectedTrackerIfAvailable(reason: "auto reconnect") {
                return
            }
    ...
    private func restore(_ restored: CBPeripheral) {
        peripheral = restored
        cacheBluetoothDevice(restored, advertisementData: [:], rssi: nil)
    ...
    private func bootstrapStoreIfNeeded() {
        ...
        storeBootstrapTask = Task { @MainActor in
            ...
            do {
                ...
                try await self.mergeRenamedDeviceRowsIfNeeded(in: store)
                ...
                self.pruneFineGrainDataIfNeeded()
    ...
    private func installStore(_ store: TrackerStore) {
        self.store = store
    ...
    private func mergeRenamedDeviceRowsIfNeeded(in store: TrackerStore) async throws {
        let mergedRows = try await store.mergeDeviceData(from: Self.renamedDeviceID, into: deviceId)
        if mergedRows > 0 {
            append("Recovered \(mergedRows) rows saved under the renamed device id")
        }
    }

    private func bondWithBatteryCommand(_ characteristic: CBCharacteristic) {
        seq = seq &+ 1
        let frame = makeTracker4CommandFrame(command: 26, seq: seq, payload: [0x00])
    ...
    private func pruneFineGrainDataIfNeeded() {
        guard let store else { return }
        let cutoff = Int(Date().timeIntervalSince1970) - Self.fineGrainRetentionSeconds
        guard cutoff > 0 else { return }
        Task { @MainActor in
            do {
                let result = try await store.pruneFineGrainData(deviceId: self.deviceId, olderThan: cutoff)
                guard result.totalRows > 0 else { return }
                self.append("Pruned \(result.totalRows) finalized fine-grain rows older than 72h")
                if result.totalRows > 25_000 {
                    try? await store.compactStorage()
                }
            } catch {
                self.append("Fine-grain retention prune failed: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func mergedWithLiveHeartRate(_ snapshot: IOSTrackerDeviceSnapshot) -> IOSTrackerDeviceSnapshot {
        var merged = snapshot
    ...
    private func mergeLiveHeartRateIntoSnapshot(_ snapshot: inout IOSTrackerDeviceSnapshot, bpm: Int, ts: Int) {
        guard bpm >= 30, bpm <= 220 else { return }
        let sampleDate = Date(timeIntervalSince1970: TimeInterval(ts))
        guard Calendar.current.isDateInToday(sampleDate) else { return }
    ...
    private func mergeDailyHeartRateSample(bpm: Int, ts: Int, into snapshot: inout IOSTrackerDeviceSnapshot) {
    ...
    private func sendCommand(_ command: UInt8, payload: [UInt8], to characteristic: CBCharacteristic, on peripheral: CBPeripheral, writeType: CBCharacteristicWriteType = .withoutResponse) {
        seq = seq &+ 1
        let frame = makeTracker4CommandFrame(command: command, seq: seq, payload: payload)
    ...
    private func makeTracker4CommandFrame(command: UInt8, seq: UInt8, payload: [UInt8]) -> [UInt8] {
...
extension IOSTrackerScanner: @preconcurrency CBCentralManagerDelegate {
    func centralManagerDidUpdateState(_ central: CBCentralManager) {
        ...
        case .poweredOn:
            ...
            if peripheral == nil || peripheral?.state == .disconnected {
                _ = reconnectConnectedTrackerIfAvailable(reason: "Bluetooth restored")
            }
    ...
    func centralManager(_ central: CBCentralManager, willRestoreState dict: [String: Any]) {
        guard let peripherals = dict[CBCentralManagerRestoredStatePeripheralsKey] as? [CBPeripheral],
              let restored = peripherals.first(where: { Self.looksLikeTracker(name: $0.name, advertisementData: [:]) }) ?? peripherals.first else {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        guard Self.looksLikeTracker(name: peripheral.name, advertisementData: advertisementData) else { return }
        cacheBluetoothDevice(peripheral, advertisementData: advertisementData, rssi: RSSI)
...
extension IOSTrackerScanner: @preconcurrency CBPeripheralDelegate {
